...
WITH country_yearly AS (
    SELECT 
        country,
        year,
        SUM(total_energy_twh) AS energy
    FROM cleaned_energy_data
    GROUP BY country, year
)
SELECT 
    country,
    STDDEV(energy) AS volatility,
    RANK() OVER (ORDER BY STDDEV(energy) DESC) AS volatility_rank
FROM country_yearly
GROUP BY country;
...
WITH yearly_data AS (
    SELECT 
        year,
        SUM(total_energy_twh) AS energy,
        SUM(carbon_emissions_mt) AS emissions
    FROM cleaned_energy_data
    GROUP BY year
),
growth AS (
    SELECT 
        ...
        energy,
        emissions,
        (energy - LAG(energy) OVER (ORDER BY year)) / LAG(energy) OVER (ORDER BY year) AS energy_growth,
        (emissions - LAG(emissions) OVER (ORDER BY year)) / LAG(emissions) OVER (ORDER BY year) AS emission_growth
    FROM yearly_data
)
SELECT 
    year,
    ROUND(energy_growth, 3) AS energy_growth,
    ROUND(emission_growth, 3) AS emission_growth,
    ROUND(energy_growth - emission_growth, 3) AS decoupling_index
FROM growth;
